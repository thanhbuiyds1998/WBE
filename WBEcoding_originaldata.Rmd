...
WBE= WBE%>%
  mutate(Weekday= wday(Date,label=T,abbr = T),
         ...
         NICabs_COT= COTmassavg15*2.85,
         Tobacco_COT = NICabs_COT/(0.172*0.9))
```
...
```{r}
attach(WBE)
table1(~COTmassavg15+ACEmassavg+PARAmassavg+ NICabs_COT+Tobacco_COT|Period, data= WBE)
#Nicotine
...
```{r}
#Nicotine
m=ggplot(WBE, aes(x=Weekday, y= NICabs_COT, fill=Weekday))+ geom_boxplot() + labs(y="Nicotine consumption (mg/day/person)",x=NULL)+
